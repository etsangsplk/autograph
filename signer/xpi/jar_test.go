...
func TestMakeManifestsAndRepackJarMatchPythonSigningClients(t *testing.T) {
	...
	manifest, sigfile, err := makeJARManifests(testJar)
	if err != nil {
	...
	var (
		...
		repackedManifest = mustReadFileFromZIP(repacked, "META-INF/manifest.mf")
		signedManifest = mustReadFileFromZIP(signedJar, "META-INF/manifest.mf")
	)
